...
function Provider (options) {
    ...
    this.path = options.path||c.providers[options.provider].path||'';
...
Provider.prototype.createPath = function (api, options) {
    ...
    switch (true) {
        ...
        case this.trello: case this.dropbox:
        ...
        case this.google:
            var name = options.api||this.api,
                _api = c.providers.google.api[name],
                version = options.version||_api.version,
                path = _api.path||name;

            return version
                ? [path, version, api].join('/')
                : [path, api].join('/');

        // 
        case this.yahoo:
            var name = options.api||this.api,
                _api = c.providers.yahoo.api[name],
                version = options.version||_api.version;
